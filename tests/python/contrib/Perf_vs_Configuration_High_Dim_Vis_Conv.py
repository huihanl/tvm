...
def read_and_create_dataset(filepath, MB, IC, OC, IN_DIM, G, K, stride, pad):
    ...
    with open(filepath) as fp:
        for _, line in enumerate(fp):
            ...
            line = eval(line)
            config = [line["i"][5]["e"][i][2] for i in range(6)]
            runtime = line["r"][0][0]
    ...
    """
    new_config_lst = config_lst
    new_runtime_lst = runtime_lst
    ...
    new_config_lst = pd.DataFrame(new_config_lst, columns=["mcb", "ncb", "kcb", "mr", "nr", "16"])
    new_config_lst = new_config_lst.drop(["16"], axis = 1)
